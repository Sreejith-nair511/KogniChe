...
`01_YOUR_DATA_MODEL.pdf` lists all 15 tables you have been given — 13 that this statement lives in, plus reference tables — with every field.
...
- **Run `data/queries/starter_queries.sql`.** 6 queries, each with a comment explaining what it shows, all of them running as-is against `data/APS-04.db`:

```bash
sqlite3 data/APS-04.db < data/queries/starter_queries.sql
```

That is 28,630 rows of real, queryable travel data. It is fully synthetic — no real people, no personal data — so it is safe to commit to a public repository.
...
| `data/APS-04.db` | SQLite, indexed, only your tables. No setup. |
